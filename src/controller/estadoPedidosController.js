...
const {existeEstado,buscarId,modificarEstado} = require('../services/sevirceEstadoPedido')

async function obtenerEstadosPedidos( req= request, res = response){
  try {
    const query = {estado:true};
    const estadosUsuariosget = await estadopedido.find(query);
...
async function crearNuevoEstado(req= request, res = response){
  try {
    ...
    const estado_encontrado = await existeEstado(nombre); 
     if(estado_encontrado) return res.status(409). json({msg: "Ya existe el estado ingresado!"}); 
     
     const nuevoEstado = new estadopedido({nombre, estado});
...
async function modificarEstados(req= request, res = response){
  try {
    ...
    const isUpdateOk = await modificarEstado(id,estadoModificado); 
    if(isUpdateOk){
...
async function eliminarEstado(req= request, res = response){
  try {
    ...
    const isDeleteOk = await modificarEstado(id,estadoModificado); 
    if(isDeleteOk){
...
async function obtenerEstadoPedido(req= request, res = response){
  try {
